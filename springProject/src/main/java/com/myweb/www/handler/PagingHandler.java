...
public class PagingHandler {
	...
	public PagingHandler(PagingVO pgvo, int totalCount) {
		...
		this.endPage = (int)Math.ceil(pgvo.getPageNo() / (double)pgvo.getQty()) * pageQty;
		...
		//전체 글수에서 / 한 페이지에 표시되는 게시글 수 ceil로 올림
		realEndPage = (int)Math.ceil(totalCount / (double)pgvo.getQty());
		
		/*
		 * totalCount는 73이며, pgvo.getQty()는 10으로 설정합니다.
		 * 
		 * (int)Math.ceil(73 / (double)10)은 다음과 같이 계산됩니다:
		 * 
		 * 73을 10으로 나누면 7.3이 됩니다. (double)를 사용하여 10을 부동 소수점 수로 캐스팅하면, 나눗셈 결과 역시 부동 소수점
		 * 수가 됩니다. Math.ceil() 함수를 사용하여 7.3을 올림하면 8.0이 됩니다. 그 결과를 정수로 캐스팅하면 8이 됩니다.
		 * 마지막으로, 이 값인 8이 realEndPage 변수에 할당됩니다.
		 * 
		 * 따라서 위의 코드에서 realEndPage 변수에 할당된 값은 8입니다. 이것은 totalCount가 73이고 pgvo.getQty()가
		 * 10으로 설정된 경우 해당 코드가 계산한 결과입니다.
		 */
